	...
	public static int wordScore(String word) {
		for (int i = 0; i< word.length(); i++){
			if (!(word.charAt(i) >= 'A' && word.charAt(i) <= 'Z') && !(word.charAt(i) >= 'a' && word.charAt(i) <= 'z')) {
    			System.out.println("Error: Invalid word '" + word + "'. Only letters!");
    			return 0;
				}
		}
		...
		int score = 0;
		for (int i = 0 ; i<word.length(); i++){
			score = score + SCRABBLE_LETTER_VALUES[(int)word.charAt(i) - 'a'];
		}
		score = score * word.length();
		if (word.indexOf('r') >=0 && word.indexOf('u') >= 0 && word.indexOf('n') >= 0 && word.indexOf('i') >= 0){
			score = score + 1000;
		}
